...
export const joinQueue = onCall({ region: REGION }, async (request) => {
...
export const leaveQueue = onCall({ region: REGION }, async (request) => {
  const uid = request.auth?.uid;
  if (!uid) throw new HttpsError('unauthenticated', 'Giriş yapmalısın.');

  const { entryKey } = request.data;
  if (!entryKey) throw new HttpsError('invalid-argument', 'entryKey gerekli.');
  ...
  if (entry && entry.playerId !== uid) {
    throw new HttpsError('permission-denied', 'Bu giriş sana ait değil.');
  }
...
export const tryMatch = onCall({ region: REGION }, async (request) => {
  const uid = request.auth?.uid;
  if (!uid) throw new HttpsError('unauthenticated', 'Giriş yapmalısın.');
  ...
  const txResult = await queueRef.transaction((queue: Record<string, any> | null) => {
    ...
    if (!foundKey || !foundId) return; // abort
